import { Injectable, Inject, Optional, StaticProvider, StaticClassProvider } from '@angular/core'
...
).find((provider: StaticClassProvider) => provider.provide === PlatformLocation).useClass
...
export class ExpressRedirectPlatformLocation extends ServerPlatformLocation {
  ...
  private redirectExpress(state: any, title: string, url: string) {
    ...
    if (this.res.finished) {
      ...
      console.warn('Attempted to redirect on a finished response. From', this.req.url, 'to', url)
